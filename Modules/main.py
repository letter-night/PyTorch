...
Use 'argparse' module to be able to customize hyperparameter values for
training procedues.
"""
...
print(f"[INFO] Training a model for {NUM_EPOCHS} epochs with batch size {BATCH_SIZE} using {HIDDEN_UNITS} hidden units and a learning rate of {LEARING_RATE}")
...
engine.train(model=model, train_dataloader=train_dataloader, test_dataloader=test_dataloader,
			 loss_fn=loss_fn, optimizer=optimizer, epochs=NUM_EPOCHS, device=device, writer=None)
...
loaded_model.load_state_dict(torch.load("models/tinyvgg_model.pth"))

test_image_path_list = list(Path(test_dir).glob("*/*.jpg"))
test_image_path_sample = random.sample(population=test_image_path_list, k=3)

for image_path in test_image_path_sample:
	utils.pred_and_plot(model=loaded_model, image_path=image_path, 
					 class_names=class_names, transform=data_transforms, device=device)


""" command line...
python main.py --num_epochs 5 --batch_size 128 --hidden_units 128 --learning_rate 0.0003
"""